import { TileType, FurnitureType, DEFAULT_COLS, DEFAULT_ROWS, TILE_SIZE, Direction } from '../pixel-types'
import type { TileType as TileTypeVal, OfficeLayout, PlacedFurniture, Seat, SeatPosition, FurnitureInstance, FloorColor } from '../pixel-types'
...
export function layoutToFurnitureInstances(furniture: PlacedFurniture[]): FurnitureInstance[] {
  ...
  for (const item of furniture) {
    ...
    instances.push({ sprite, type: item.type, x, y, zY })
  }
  return instances
}
...
export function getBlockedTilesFromGids(
  gidLayers: number[][],
  cols: number,
  blockingLayerIndices: number[],
  excludeTiles?: Set<string>,
): Set<string> {
  const tiles = new Set<string>()
  for (const layerIdx of blockingLayerIndices) {
    const layer = gidLayers[layerIdx]
    if (!layer) continue
...
/** Create seats from numbered positions (seat-1, seat-2, etc.) */
export function seatsFromPositions(positions: SeatPosition[]): Map<string, Seat> {
  const seats = new Map<string, Seat>()
